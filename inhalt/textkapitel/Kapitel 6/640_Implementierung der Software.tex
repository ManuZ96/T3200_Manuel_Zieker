...
In \texttt{setup()} werden Hardware, Sensorik, WLAN, Zeitsynchronisation, Wetterdaten und Webserver initialisiert (vgl. Skript~\ref{lst:Setup}).
...
In \texttt{Loop()} Webserver-Anfragen verarbeitet. Die Funktion wird zyklisch wiederholt um alle Anzeigen zu aktualisieren wenn es eine Veränderung der Werte gibt (vgl. Skript~\ref{lst:Loop} und ~\ref{lst:Setup}).
